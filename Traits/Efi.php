...
namespace Modules\Efi\Traits;

use App\Traits\DateTime as TraitDateTime;
use Efi\EfiPay;
use Efi\Endpoints as EfiEndpoints;
...
trait Efi
{
    ...
    private function newInstance(array $options = []): void
    {
        ...
        $this->api = EfiPay::getInstance($current_options);
    }
    ...
    {
        $app_setting = setting();
        $pix_cert = $app_setting->get('efi.pix_cert');

        if($pix_cert == '-----FAKE CERTIFICATE-----') {
            return;
        }

        $webhook_secret = $app_setting->get('efi.webhook_secret');
        if(empty($webhook_secret)) {
            $webhook_secret = Str::uuid()->toString();

            $app_setting->set('efi.webhook_secret', $webhook_secret);
            $app_setting->save();
        }

        $webhook_registered = null;
        $webhook = url(company_id() . '/efi/webhook/' . $webhook_secret);
        // $webhook = 'https://test.com/' . company_id() . '/efi/webhook/' . $webhook_secret;
    ...
    public function getCertExpiry() {
        $certExpiry = null;
        $certTimestamp = setting('efi.cert_timestamp');
        if ($certTimestamp !== null) {
            ...
            if($howManyMonths <= 4) {
                $alertColor = 'orange';
                $transKey = 'efi::general.cert_expiry_warning';
                ...
                $replace = [
                    ...
                    'url_setting' => route('settings.module.edit', ['efi'])
                ];

                if($howManyMonths <= 2) {
                    $alertColor = 'red';
                    $transKey = 'efi::general.caution_cert_expiry';
                }
